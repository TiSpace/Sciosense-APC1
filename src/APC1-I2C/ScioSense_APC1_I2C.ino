...
#define sensorAPC_sleep 100      // sleep time in seconds
#define sensorAPC_active 10  //active time between sleep
...
void setup() {
  ...
  apc1.begin(false);  //(false);    // calling apc1.begin() is causing instability on startup with ENS160
  delay(5);
...
#ifndef APC1_NOSLEEP
  if ((timeLastWakeUp + (sensorAPC_sleep * 1000U)) < millis()) {
    ...
    if ((timeLastWakeUp + (sensorAPC_sleep * 1000U) + (sensorAPC_active * 1000)) < millis()) {
