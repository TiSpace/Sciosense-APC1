...
// **************************************************************************
bool ScioSense_APC1::begin(bool debug) {
  bool result;
  uint8_t i2cbuf[2];
...
bool ScioSense_APC1::checkPartID(void) {
  ...
  Serial.println(i2cbuf[20], HEX);

Serial.print("Serialnumber: ");
for (i=10;i<18;i++){
  Serial.print( i2cbuf[i],HEX);Serial.print(" ");
}
...
Serial.println();
  ...
  if (debugAPC1) {
    Serial.println("APC1 I2C module deteced successfully!");
  }
...
bool ScioSense_APC1::measure(void) {
  ...
  if (debugAPC1) Serial.println("Start measurement");
...
// Erase ENS160 baseline in the chip
bool ScioSense_APC1::EraseBaseline(void) {
